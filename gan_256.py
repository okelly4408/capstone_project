...
class gen(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
        	nn.utils.spectral_norm(nn.ConvTranspose2d(nz, ngf * 32, 4, 1, 0, bias=False)),
            nn.LeakyReLU(0.2, True),
            nn.BatchNorm2d(ngf * 32),
            # input is Z, going into a convolution
            nn.utils.spectral_norm(nn.ConvTranspose2d(ngf * 32, ngf * 16, 4, 1, 0, bias=False)),
            nn.LeakyReLU(0.2, True),
...
class disc(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.utils.spectral_norm(nn.Conv2d(ndf * 16, ndf * 32, 4, stride=2, padding=1, bias=False)),
            nn.LeakyReLU(0.2, inplace=True),
	...
	for epoch in range(num_epochs):
	    for i, data in enumerate(dataloader, 0):
	        ...
	        if (iters % 25 == 0) or ((epoch == num_epochs-1) and (i == len(dataloader)-1)):
	            ...
	            vutils.save_image(vutils.make_grid(fake[0], padding=2, normalize=True), 'nnimages/'+str(iters)+'image' + str(random.random()) + '.jpg')
	        iters += 1
	torch.save({
...
def run():
	...
	for i in range(256):
		...
		vutils.save_image(vutils.make_grid(nn_img[0], padding=2, normalize=True), 'nnimages/z_'+str(i)+'image' + str(random.random()) + '.jpg')
